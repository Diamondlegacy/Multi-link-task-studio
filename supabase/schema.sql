...
('video-quality-eval', 'Video Quality Eval', 'video',
 ...
 'Evaluate this video''s visual quality: sharpness, motion smoothness, lighting, and any artifacts or glitches. Note if audio is present and its clarity. Rate 1-10 and explain your reasoning in 2-3 sentences.',
 3)
...
-- 6. Storage bucket for large audio/video uploads
-- (private — files aren't public, only the logged-in owner and
--  the server-side Edge Function can access them)
...
create policy "users upload own files" on storage.objects
  for insert with check (
    bucket_id = 'task-uploads' and (storage.foldername(name))[1] = auth.uid()::text
  );

-- Users can only read their own uploaded files back
